...
static void temp_cal(float val) {
//	displayParams.brightness = val*100;
//	updateColorTempDisplay( &displayParams);
	//LOG("\n volt:%d.%d",(uint16)val,((uint16)(val*1000))%1000);
	float res_ntc = val * 1000 * TEMP_RES_PULLUP / TEMP_POWER_VOLTAGE;
	//LOG("\n ntc:%d.%d K",(uint16)res_ntc,((uint16)(res_ntc*10))%10);
	...
	for (uint8 i = 0; i < sizeof(table_res) / sizeof(table_res[0]); i++) {
		if (res_ntc > table_res[i]) {
			...
			//LOG("\n temp_value:%d",(uint16)(temp_value*1000));
			return;
		}
	}
	temp_value = 0;
