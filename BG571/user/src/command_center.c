...
//		temperatureDisplay(64,4,batt_vol);
		...
		if (TESTMODE&&TESTMODE!=BurnInTest) {
			//temperatureDisplay(2, 0, batt_perc);
			//temperatureDisplay_char(40, 0, 102);
...
//				temperatureDisplay(0, 3, batt_vol);
			...
			if (Flag_Status_Charge == Battery_fully) {
				...
				uint8 fIsNeedUpdateBattLv = 0;
//				displayFloat(0,2,batt_vol*0.001,3,'v');
				chkLightEffectModeVoltCompensation(&vBattVolt, &fIsNeedUpdateBattLv);
				//LOG("\n vol=%d.%d\n",(uint16)vBattVolt,((uint16)(vBattVolt*1000))%1000);
//				displayFloat(0,4,vBattVolt,3,'v');
				//temperatureDisplay(0,3, (uint16)(vBattVolt*1000));
				if (fIsNeedUpdateBattLv){
					battInfo.vBattPercent=setBattLevel(&tempLv, &vBattVolt, &battInfo.vCurrentBattLv);
				}
				//LOG("\n vol_comp=%d.%d\n",(uint16)vBattVolt,((uint16)(vBattVolt*1000))%1000);

				//batt_perc=setBattLevel(&tempLv, &vBattVolt, &battInfo.vCurrentBattLv);
...
//				CCS_Systems_off();
	...
	if (keys & KEY_FUNC) {
		...
		//Function switch
		if (KEY_FUNC_Press_times && CCS_MODE_SYSTEM) {
	...
	if (CCS_MODE_SYSTEM&&!fIsInvalidMacAddr) {
		...
		//light up
		if (keys & KEY_LIGHTUP) {
			if (KEY_LIGHTUP_Press_times ++> 20) {
				if (KEY_LIGHTUP_Press_times % 2) {
					keyBrightnessIncProcess(&flag);
				}
			}
		} else if (KEY_LIGHTUP_Press_times) {
		...
		//light down
		if (keys & KEY_LIGHTDOWN) {
			if (KEY_LIGHTDOWN_Press_times ++> 20) {
				if (KEY_LIGHTDOWN_Press_times % 2) {
					keyBrightnessDecProcess(&flag);
				}
			}
		} else if (KEY_LIGHTDOWN_Press_times) {
...
//	}
	...
	uint16 keys = keys_read();
	if (keys & KEY_FUNC && keys & KEY_LIGHTUP) {
		if (keys& KEY_TRUNDOWN) {
